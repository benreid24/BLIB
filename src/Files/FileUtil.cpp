...
std::vector<std::string> FileUtil::listDirectory(const std::string& path, const std::string& ext, bool recursive) {
    if (path.empty())
        return {};
    ...
    while (cd != nullptr) {
        while ((cfile = readdir(cd))) {
            ...
            if (file.find(".") != std::string::npos) {
                ...
                const std::vector<std::string> files = listDirectory(joinPath(folder, file), ext, true);
