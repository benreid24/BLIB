...
unsigned int TextEntryComponent::findCursorPosition(const sf::Vector2f& overlayPos) const {
    ...
    if (lp.x < owner.getAcquisition().width * 0.5f) { return owner.getLineStartIndex(line); }
...
void TextEntryComponent::positionItems() {
    ...
    if ((bounds.width + bounds.left) > acq.width || (bounds.top + bounds.height) > acq.height) {
        ...
        const float right = acq.width - Padding * 2.f;
        if (gcpos.x < 0.f) { textOffset.x = -cpos.x; }
        else if (cpos.x > right) {
            textOffset.x = right - cpos.x - g.bounds.width - g.advance - CaratWidth;
        }
        ...
        const float bottom = acq.height - Padding * 2.f;
