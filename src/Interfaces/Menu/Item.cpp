...
void Item::visit(Visitor visitor, int x, int y, std::list<std::pair<int, int>>& visited) const {
    if (std::find(visited.begin(), visited.end(), std::make_pair(x, y)) != visited.end()) return;
