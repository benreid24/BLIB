...
layout(constant_id = 0) const int msaaSampleCount = 1;

void main() {
    float totalOcclusion = 0.0;
    ivec2 uv = ivec2(inUV);

    for (int sampleId = 0; sampleId < msaaSampleCount; ++sampleId) {
        vec3 pos = texelFetch(positions, uv, sampleId).xyz;
        vec3 normal = texelFetch(normals, uv, sampleId).xyz;
        ...
        for (uint i = 0; i < ssaoParams.sampleCount; ++i) {
            ...
            float sampleDepth = texelFetch(positions, ivec2(ndcOffset.xy), sampleId).z;
    ...
    outColor = totalOcclusion / float(msaaSampleCount);
}
