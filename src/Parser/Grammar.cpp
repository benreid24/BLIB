...
bool Grammar::compile() {
    ...
    Node::Sequence generated;
    for (const auto r : reductionTable) {
        if (r.second.reductionsPossible == 1 && r.second.result != Node::None) {
            if (std::find(generated.begin(), generated.end(), r.second.result) !=
                generated.end()) {
                std::cerr << "Error: Grammar is ambiguous. Rule " << r.second.result
                          << " is generated by multiple sequences\n";
                reductionTable.clear();
                return false;
            }
            generated.push_back(r.second.result);
        }
    }
