#ifndef BLIB_UTIL_RANGEALLOCATOR_HPP
#define BLIB_UTIL_RANGEALLOCATOR_HPP
...
public:
     ...
     *
     * @param capacity The capacity to allocate ranges within
     */
    RangeAllocator(T capacity);
...
template<typename T>
RangeAllocator<T>::RangeAllocator(T cap)
: totalSize(cap) {
...
template<typename T>
typename RangeAllocator<T>::Range RangeAllocator<T>::alloc(T size) {
    ...
    if (allocFrom) {
        const Range result(allocFrom->start, size);
    ...
    // no sufficiently large range exists
    return Range(0, 0);
}

template<typename T>
void RangeAllocator<T>::release(Range range) {
...
template<typename T>
void RangeAllocator<T>::releaseAll() {
    freeRanges.clear();
    freeRanges.emplace_back(0, totalSize);
}

template<typename T>
void RangeAllocator<T>::releaseAllAndResize(T newSize) {
    totalSize = newSize;
    releaseAll();
}

template<typename T>
T RangeAllocator<T>::poolSize() const {
    return totalSize;
}
