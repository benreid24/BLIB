...
public:
     ...
     */
    void willCall(const Callback& cb);
     ...
     */
    void willAlwaysCall(const Callback& cb);
...
template<typename... CallbackArgs>
void Signal<CallbackArgs...>::willCall(const Callback& cb) {
    userCallbacks.push_back(cb);
}

template<typename... CallbackArgs>
void Signal<CallbackArgs...>::willAlwaysCall(const Callback& cb) {
...
void Signal<CallbackArgs...>::operator()(CallbackArgs&&... args) {
    ...
    for (const auto& cb : userCallbacks) { cb(std::forward<CallbackArgs>(args)...); }
